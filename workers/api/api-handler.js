...
// caché Server-Side integrada + Request Coalescing + ODESLI LAZY LOAD
// ===============================================================
...
async function getOdesliLinks(spotifyUrl) {
  try {
    ...
    const headers = {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; AppleWebKit/537.36.0; (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36.0' };
...
// ===============================================================
//  SPOTIFY HANDLER (LAZY LOAD INTEGRADO)
// ===============================================================
async function handleSpotifyRequest(request, env, ctx) {
  try {
    ...
    const linksOnlyParam = url.searchParams.get("linksOnly") === "true"; // <-- NUEVO: Chequeo de Lazy Load
    ...
    if (searchData && searchData.tracks.items.length > 0) {
      ...
      // =================================================================
      // CACHÉ KV + COALESCING + LÓGICA LAZY LOAD
      // =================================================================
      try {
      ...
      const resp = {
        ...
        isrc: trackIsrc,
        links: null, // <-- IMPORTANTE: Se llena abajo si es exitoso
      ...
      // =======================================================================
      // LÓGICA FINAL DE ODESLI (Lazy Load)
      // =======================================================================
      if (odesliResult.success) {
        // Éxito: Asignamos links a la respuesta
        resp.links = {
