...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        let currentCredits = "";
        // FIX pausa/reanudación: registra cuándo se pausó para compensar trackStartTime
        let pausedAt = 0;
        // updateTimer expuesta al scope para poder reiniciarla al reanudar
        ...
        function showInstallInvitation() {
            ...
            if (localStorage.getItem('rm_pwa_invite_dismissed') === 'true') {
                return;
            }
            let os = 'other';
            ...
            let activeBtn = null;
            if (os === 'android') {
                activeBtn = installAndroidBtn;
            } else if (os === 'ios') {
                activeBtn = installIosBtn;
            } else if (os === 'windows') {
                activeBtn = installWindowsBtn;
            }
                ...
                setTimeout(() => {
                    if (!isPlaying) {
                        ...
                        audioPlayer.play().then(() => {
                            ...
                            updateStatus(true);
                            startTimeStuckCheck();
                            // FIX: compensar trackStartTime por el tiempo que estuvo sin audio
                            if (pausedAt > 0 && trackStartTime > 0) {
                                trackStartTime += (resumeStart - pausedAt);
                                pausedAt = 0;
                            }
                            // Reiniciar el loop del timer si quedó muerto
                            if (animationFrameId) cancelAnimationFrame(animationFrameId);
                            if (updateTimer) updateTimer();
                            // Detectar cambios ocurridos durante la pausa
                            updateSongInfo(true);
        ...
        class CustomSelect {
            ...
            toggle() {
                this.customSelectWrapper.classList.toggle('open');
            }
            open() {
                this.customSelectWrapper.classList.add('open');
            }
            close() {
                this.customSelectWrapper.classList.remove('open');
            }
            ...
            updateTriggerText() {
                ...
                if (st) {
                    txt = st.service === 'radioparadise' ? (st.name.split(' - ')[1] || st.name) : st.name;
                }
        ...
        function displayAlbumCoverFromUrl(url) {
            ...
            img.onerror = function () {
                console.warn('Error al cargar portada:', url);
                resetAlbumCover();
            };
            img.src = url;
        }
        ...
        async function loadStations() {
            try {
                ...
                for (const n in grouped) {
                    grouped[n].sort((a, b) => a.name.localeCompare(b.name));
                }
                if (loadingStations) {
                ...
                logErrorForAnalysis('Load error', {
                    error: e.message,
        ...
        if (stationSelect) {
            stationSelect.addEventListener('change', function() {
                if (this.value) {
                    ...
                    if (st) {
                        // Cambio de estación: resetear pausedAt para no tomar como reanudación
                        pausedAt = 0;
                        currentStation = st;
                        stationName.textContent = st.service === 'radioparadise' ? (st.name.split(' - ')[1] || st.name) : st.name;
                        showWelcomeScreen();
        ...
        // ==========================================================================
        // FIX: LISTENER PLAY — distingue PAUSAR / REANUDAR / NUEVA ESTACIÓN
        // ==========================================================================
        if (playBtn) {
            playBtn.addEventListener('click', function() {
                ...
                if (isPlaying) {
                    ...
                    isPlaying = false;
                    updateStatus(false);
                    pausedAt = Date.now(); // registrar momento de pausa
                    wasPlayingBeforeFocusLoss = false;
                    stopPlaybackChecks();
                    // NO limpiar updateInterval: los pollers siguen corriendo en background
                    // para detectar cambios de canción mientras está pausado.

                } else if (currentStation && currentTrackInfo && pausedAt > 0) {
                    // ── REANUDAR — misma estación ──
                    const resumeStart = Date.now();
                    audioPlayer.play().then(() => {
                        isPlaying = true;
                        updateStatus(true);
                        ...
                        // Compensar trackStartTime por el tiempo pausado
                        if (trackStartTime > 0) {
                            trackStartTime += (resumeStart - pausedAt);
                        }
                        pausedAt = 0;

                        // Reiniciar el loop del timer (quedó muerto al pausar)
                        if (animationFrameId) cancelAnimationFrame(animationFrameId);
                        if (updateTimer) updateTimer();

                        // Chequeo inmediato: puede haber cambiado canción mientras estuvo pausado
                        updateSongInfo(true);
                        ...
                        console.warn("Play rechazado al reanudar:", error);
                        handlePlaybackError();
                    });

                } else {
                    // ── PRIMERA VEZ o NUEVA ESTACIÓN ──
                    if (currentStation) {
                        pausedAt = 0;
                        playStation();
                    } else {
        ...
        async function updateSongInfo(bypassRateLimit = false) {
            if (!currentStation || !currentStation.service) {
                return;
            }
            if (isUpdatingSongInfo) {
                return;
            }
            if (currentStation.service === 'somafm') {
            ...
            try {
                ...
                if (data.songs && data.songs.length > 0) {
                    ...
                    if (isNew) {
                        ...
                        trackDuration = 0;
                        // Si está pausado, actualizar pausedAt para que la compensación
                        // al reanudar use el nuevo trackStartTime como referencia
                        if (pausedAt > 0) {
                            pausedAt = Date.now();
                        }
                        startCountdown();
        ...
        async function updateRadioParadiseInfo(bypassRateLimit = false) {
            ...
            try {
                ...
                if (isNew) {
                    ...
                    // Si está pausado, actualizar pausedAt para sincronizar la compensación
                    if (pausedAt > 0) {
                        pausedAt = Date.now();
                    }
                    startCountdown();
        ...
        function startRadioParadisePolling() {
            ...
            const rpLoop = async () => {
                if (!currentStation || currentStation.service !== 'radioparadise') {
                    return;
                }
        ...
        // FIX: sanitizar album — eliminar sufijos ":N" que generan 404 en el Worker
        // =======================================================================
        async function fetchSongDetails(artist, title, album, fetchId) {
            ...
            const sAl = album
                ? album
                    ...
                    .replace(/:\d+$/, "") // FIX: elimina sufijos tipo ":1", ":2" que rompen la búsqueda
                    .trim()
                : "";
            ...
            try {
                ...
                logErrorForAnalysis('Worker/Spotify error', {
                    error: e.message,
                    artist: sA,
                    title: sT,
                    timestamp: new Date().toISOString()
        ...
        async function getMusicBrainzCredits(artist, title, isrc, fetchId) {
            ...
            try {
                    ...
                    try {
                        ...
                        if (res.ok) {
                            ...
                            if (data.recordings && data.recordings.length > 0) {
                                ...
                                updateCreditsUI(r.relations, fetchId);
                                return;
                            }
                        }
                    } catch (isrcError) {}
                ...
                if (d.recordings && d.recordings.length > 0) {
                    recordingId = d.recordings[0].id;
                }

                if (recordingId) {
                    await new Promise(resolve => setTimeout(resolve, 1100));
                    if (fetchId !== currentSongFetchId) return;
                    const creditsRes = await fetch('https://musicbrainz.org/ws/2/recording/' + recordingId + '?inc=artist-rels&fmt=json');
                    if (creditsRes.ok) {
                        const creditsData = await creditsRes.json();
                        updateCreditsUI(creditsData.relations, fetchId);
                    }
                }
            } catch (e) {
                logErrorForAnalysis('MusicBrainz credits error', {
                    error: e.message,
                    artist,
                    title,
                    timestamp: new Date().toISOString()
        ...
        // ==========================================================================
        // UTILIDAD: actualiza el tooltip de créditos en la UI
        // ==========================================================================
        function updateCreditsUI(relations, fetchId) {
            if (fetchId !== currentSongFetchId) return;
            const creditsElement = document.getElementById('trackCredits');
            if (!creditsElement) return;
            const artistRelations = (relations || []).filter(rel => rel.type && rel.artist);
        ...
        // ==========================================================================
        // FIX: startCountdown — updateTimer asignada al scope externo
        // para poder reiniciar el loop al reanudar desde el listener del botón
        // ==========================================================================
        function startCountdown() {
            ...
            // FIX: asignada a la variable del scope externo (no declarada localmente)
            // Esto permite llamar a updateTimer() desde el listener del botón al reanudar
        ...
        if (audioPlayer) {
            ...
            // FIX: stalled — verificar estado antes de reanudar + silenciar AbortError
            audioPlayer.addEventListener('stalled', () => {
                if (isPlaying) {
                    wasPlayingBeforeFocusLoss = true;
                    setTimeout(() => {
                        if (!wasPlayingBeforeFocusLoss || !currentStation || !isPlaying) return;
                        audioPlayer.play()
                            .then(() => {
                                isPlaying = true;
                                updateStatus(true);
                            ...
                            .catch(e => {
                                if (e.name !== 'AbortError') {
                                    console.error("Error al reanudar tras stalled:", e);
                                }
                            });
        ...
        if ('mediaSession' in navigator) {
            ...
            navigator.mediaSession.setActionHandler('play', () => {
                if (!isPlaying && currentStation) {
                    // Reutilizar lógica de reanudación
                    if (currentTrackInfo && pausedAt > 0) {
                        const resumeStart = Date.now();
                        audioPlayer.play().then(() => {
                            isPlaying = true;
                            updateStatus(true);
        ...
        if (installPwaBtnIos) {
            installPwaBtnIos.addEventListener('click', (e) => {
                e.preventDefault();
                showNotification('Para instalar en iOS: Pulsa el botón Compartir y luego Añadir a pantalla de inicio.');
            });
        }
        ...
        document.addEventListener('keydown', function(event) {
            if (!document.querySelector('.custom-select-wrapper.open') &&
                ...
                let indicator = document.getElementById('key-search-indicator');

                if (!indicator) {
                    indicator = document.createElement('div');
                    indicator.id = 'key-search-indicator';
                    indicator.className = 'key-search-indicator';
                ...
                clearTimeout(wrapper.keyIndicatorTimeout);
                wrapper.keyIndicatorTimeout = setTimeout(() => {
        ...
        if (versionSpan) {
            fetch('/sw.js')
                ...
                .then(t => {
                    ...
                    if (m && m[1]) {
                        versionSpan.textContent = m[1];
                    } else {
                        versionSpan.textContent = 'N/D';
                        console.warn('No version match in sw.js');
                    }
                })
                .catch(e => {
                    console.error('Error loading sw version:', e);
        ...
        if (le) {
            le.textContent = 'Error critico: ' + error.message + '.';
